...
Trace 목록 API의 서비스, 오류, duration, Cursor 조건이 실제 DB 작업량을 줄이는지 확인하고, 후보 Trace를 먼저 찾는 대안 SQL의 적용 가능성을 검증했다.
...
* 서비스: 8개
* 오류 Trace: 2,000개
* 50ms 이상 Span 포함 Trace: 1,000개
* 250ms 이상 Span 포함 Trace: 200개

### 기존 SQL 측정

* 필터 없음: 중앙값 94.006ms
* 서비스 필터: 중앙값 104.686ms
* 오류 필터: 중앙값 108.596ms
* 250ms duration 필터: 중앙값 97.209ms
* 복합 필터: 중앙값 95.811ms
* Cursor 두 번째 페이지: 중앙값 106.646ms

모든 쿼리가 109,998개 Span과 20,000개 Trace를 먼저 처리했다.

### 후보 Trace 우선 실험

* 후보 200개, 1%: 중앙값 19.819ms
* 후보 1,000개, 5%: 중앙값 34.795ms
* 후보 20,000개, 100%: 중앙값 345.467ms

후보 100%와 동일한 조건에서 기존 전체 집계 방식은 중앙값 98.048ms였다.

### 분석

후보가 적으면 후보 Trace에 속한 Span만 다시 집계하므로 성능이 개선됐다.

후보가 전체 Trace로 증가하면 후보 목록 생성, 정렬, 전체 Span 재조회와 Merge Join이 추가되어 기존 방식보다 크게 느려졌다.

후보 비율은 duration 값만으로 판단할 수 없으며 프로젝트별 데이터 분포에 따라 달라진다.

### 결정

* 현재 Repository SQL 유지
* 후보 우선 SQL 운영 적용 보류
* duration 관련 신규 인덱스 추가 보류
* 실제 운영 데이터에서 조회 병목이 확인되면 Trace Summary 구조 검토
